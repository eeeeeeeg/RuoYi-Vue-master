  ...
  <div class="container">
    <div class="left-board">
      <div class="logo-wrapper">
        <div class="logo"><img :src="logo" alt="logo" /> 任务表单构建</div>
      </div>
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="模板名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入模板名字" />
        </el-form-item>
      </el-form>
...
import { addTemplate, listTemplate } from "@/api/system/template";
let oldActiveId;
let tempActiveData;
  ...
  methods: {
    ...
    handleAddTemplate() {
      if (this.drawingList?.length == 0) {
        return this.$message.error("模板内容不可为空");
      }
      this.open = true;
    },
    submitForm() {
    ...
    // 选中模板后的回调
    handleSelected(fromTemplate) {
      console.log(fromTemplate);
      this.$confirm("将替换当前任务列表，是否确认", "提示", {
        confirmButtonText: "确认",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          this.drawingList = JSON.parse(fromTemplate.formData);
        })
        .catch(() => {});
